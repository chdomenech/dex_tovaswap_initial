...
import { Skeleton, Text, useTooltip, HelpIcon, Flex, Box, useMatchBreakpoints } from '@tovaswapui/uikit'
...
const AutoEarningsCell: React.FC<AutoEarningsCellProps> = ({ pool, account }) => {
  ...
  const labelText = t('Recent TVS profit')
    ...
    <>
      ...
      {hourDiffSinceLastAction ? (
        <>
          ...
          <Text bold>{t('TVS per hour: %amount%', { amount: earnedCakePerHour.toFixed(2) })}</Text>
