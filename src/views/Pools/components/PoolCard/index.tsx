...
import { CardBody, Flex, Text, CardRibbon } from '@tovaswapui/uikit'
...
const PoolCard: React.FC<{ pool: DeserializedPool; account: string }> = ({ pool, account }) => {
  ...
  const isCakePool = earningToken.symbol === 'TVS' && stakingToken.symbol === 'TVS'
    ...
    >
      <PoolCardHeader isStaking={accountHasStakedBalance} isFinished={isFinished && sousId !== 0}>
        <PoolCardHeaderTitle
          ...
          subTitle={isCakePool ? t('Earn TVS, stake TVS') : t('Stake %symbol%', { symbol: stakingToken.symbol })}
        />
